...
type Account struct {
	Imsi      string    `boil:"imsi" json:"imsi" toml:"imsi" yaml:"imsi"`
...
var AccountWhere = struct {
	Imsi      whereHelperstring
	CreatedAt whereHelpertime_Time
	UpdatedAt whereHelpernull_Time
	DeletedAt whereHelpernull_Time
}{
	Imsi:      whereHelperstring{field: "\"accounts\".\"imsi\""},
...
var (
	accountAllColumns            = []string{"imsi", "created_at", "updated_at", "deleted_at"}
	accountColumnsWithoutDefault = []string{"imsi", "created_at"}
	accountColumnsWithDefault    = []string{"updated_at", "deleted_at"}
	accountPrimaryKeyColumns     = []string{"imsi"}
...
func FindAccount(ctx context.Context, exec boil.ContextExecutor, imsi string, selectCols ...string) (*Account, error) {
	accountObj := &Account{}
	...
	query := fmt.Sprintf(
		"select %s from \"accounts\" where \"imsi\"=$1 and \"deleted_at\" is null", sel,
	)

	q := queries.Raw(query, imsi)
...
func (o *Account) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = "DELETE FROM \"accounts\" WHERE \"imsi\"=$1"
	} else {
		currTime := time.Now().In(boil.GetLocation())
		o.DeletedAt = null.TimeFrom(currTime)
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"accounts\" SET %s WHERE \"imsi\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func (o *Account) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindAccount(ctx, exec, o.Imsi)
	if err != nil {
		return err
	}
...
func AccountExists(ctx context.Context, exec boil.ContextExecutor, imsi string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"accounts\" where \"imsi\"=$1 and \"deleted_at\" is null limit 1)"
	...
	row := exec.QueryRowContext(ctx, sql, imsi)
...
func (o *Account) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AccountExists(ctx, exec, o.Imsi)
}
