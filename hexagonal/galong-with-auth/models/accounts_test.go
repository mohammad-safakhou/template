...
func testAccountToManyUserRoleUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, roleUserDBTypes, false, roleUserColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, roleUserDBTypes, false, roleUserColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.UserRoleUsers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadUserRoleUsers(ctx, tx, false, (*[]*Account)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadUserRoleUsers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserRoleUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testAccountToManyAddOpUserRoleUsers(t *testing.T) {
	var err error
	...
	foreigners := []*RoleUser{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, roleUserDBTypes, false, strmangle.SetComplement(roleUserPrimaryKeyColumns, roleUserColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*RoleUser{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserRoleUsers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		count, err := a.UserRoleUsers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testAccountToManySetOpUserRoleUsers(t *testing.T) {
	var err error
	...
	err = a.SetUserRoleUsers(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetUserRoleUsers(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.UserRoleUsers[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.UserRoleUsers[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testAccountToManyRemoveOpUserRoleUsers(t *testing.T) {
	var err error
	...
	err = a.AddUserRoleUsers(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveUserRoleUsers(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.UserRoleUsers) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.UserRoleUsers[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.UserRoleUsers[0] != &e {
...
var (
	accountDBTypes = map[string]string{`ID`: `integer`, `Username`: `character varying`, `Password`: `character varying`, `PhoneNumber`: `character varying`, `Email`: `character varying`, `CreatedOn`: `timestamp without time zone`, `LastLogin`: `timestamp without time zone`}
	_              = bytes.MinRead
)
